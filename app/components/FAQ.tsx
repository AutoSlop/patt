...
export default function FAQ() {
  ...
  return (
    <div className="space-y-3 max-w-2xl mx-auto">
      {faqs.map((faq, i) => (
        <div key={i} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_12px_rgba(0,0,0,0.06)] transition-all hover:border-gray-200/80">
          <button
            onClick={() => setOpen(open === i ? null : i)}
            className="w-full flex items-center justify-between px-6 py-5 text-left cursor-pointer"
          >
            <span className="font-semibold text-text pr-4 text-[0.875rem] sm:text-[0.9375rem]">{faq.q}</span>
            <svg
          ...
          {open === i && (
            <div className="faq-answer px-6 pb-5 text-gray-400 leading-relaxed text-[0.8125rem] sm:text-[0.875rem]">{faq.a}</div>
          )}
        </div>
      ))}
    </div>
